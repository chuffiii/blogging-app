...
export const BlogCard = ({
    ...
    <div className="p-4 border-b border-slate-300 pb-4 w-screen max-w-screen-md cursor-pointer">
        <div className="flex">
            ...
            <div className="font-extralight pl-1 text-sm flex justify-center flex-col">
                {authorName}
            </div>
            <div className="flex justify-center flex-col pl-1 text-sm flex justify-center flex-col">
                <Circle />
            </div>
...
export function Avatar({ name, size = "small" }: { name: string, size?: string }) {
    return (
        ...
        <div className={`relative inline-flex items-center justify-center overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600 ${size === "small" ? "w-6 h-6" : "w-10 h-10"}`}>
            <span className={`font-medium text-gray-600 dark:text-gray-300 pb-1 ${size === "small" ? "text-sm" : "text-base"}`}>{name[0]}</span>
        </div>
